...
func new_game() -> void:
	...
	if stats != null and stats.has_method("reset_week"):
		stats.call("reset_week")

	var cps: Node = get_node_or_null(CPS_PATH)
	if cps != null and cps.has_method("refresh_all"):
...
func _display_name_for_id(id: String) -> String:
	...
	if csv_loader != null and csv_loader.has_method("load_csv"):
		...
		if typeof(defs_v) == TYPE_DICTIONARY:
			...
			if defs.has(id):
				...
				var nm2: String = String(row.get("name", ""))
				if nm2 != "":
					return nm2
	return id.capitalize()

func get_member_equip(member: String) -> Dictionary:
	var equip_sys: Node = get_node_or_null("/root/aEquipmentSystem")
	if equip_sys != null and equip_sys.has_method("get_member_equip"):
...
func get_member_level(member: String) -> int:
	...
	for k in ["level", "lvl", "lv"]:
		if row.has(k) and (typeof(row[k]) == TYPE_INT or typeof(row[k]) == TYPE_FLOAT or typeof(row[k]) == TYPE_STRING):
			return int(row[k])
	return 1
	...
	if csv_loader != null and csv_loader.has_method("load_csv"):
		...
		if typeof(v) == TYPE_DICTIONARY:
			...
			for aid in defs.keys():
				...
				if String(row.get("name", "")).strip_edges().to_lower() == want:
					return row
	return {}
...
func save() -> Dictionary:
	...
	if cal != null:
		...
		var p_val: Variant = null
		var w_val: Variant = null
		if cal.has_method("get"):
			d_val = cal.get("current_date")
			p_val = cal.get("current_phase")
			w_val = cal.get("current_weekday")
	...
	if stats_sys != null and stats_sys.has_method("save"):
		var stats_data: Variant = stats_sys.call("save")
		if typeof(stats_data) == TYPE_DICTIONARY:
			payload["stats"] = stats_data as Dictionary

	# Inventory
	var inv_sys: Node = get_node_or_null("/root/aInventorySystem")
	if inv_sys != null and inv_sys.has_method("get_save_blob"):
	...
	# Sigils
	var sigil_sys: Node = get_node_or_null("/root/aSigilSystem")
	if sigil_sys != null and sigil_sys.has_method("save"):
...
func load(data: Dictionary) -> void:
	...
	if cal != null and typeof(cal_v) == TYPE_DICTIONARY:
	...
	if stats_sys != null and typeof(stats_v) == TYPE_DICTIONARY:
	...
	var inv_sys: Node = get_node_or_null("/root/aInventorySystem")
	if inv_sys != null and typeof(items_v) == TYPE_DICTIONARY:
	...
	var sigil_sys: Node = get_node_or_null("/root/aSigilSystem")
	if sigil_sys != null and typeof(sig_v) == TYPE_DICTIONARY:
	...
	if cps != null and cps.has_method("apply_save_blob"):
