...
func _draw_capture_visual() -> void:
	...
	# Draw rotation icon behind everything (200x200, bottom centered at canvas center, moved down 12px and right 2px)
	if rotation_icon:
		var rotation_size = Vector2(200, 200)
		# Position so bottom of icon is at center of canvas, moved down 12px and right 2px
		var rotation_pos = Vector2(center.x - rotation_size.x / 2.0 + 2, center.y - rotation_size.y + 12)
